...
def is_prime(user):
    if user.groups.filter(name='prime').exists():
        return True

@user_passes_test(is_prime)
def index(request):
...
def home(request):
    ...
    return render(request, 'prime/home.html',{'gallery_products':gallery_products,'product_one':product_one,'product_two':product_two,'trending_products_one':trendings_products_one,'trending_products_two':trendings_products_two,'gallery_products':Product.objects.filter(gallery_view=True)})

@user_passes_test(is_prime)
def view_products(request):
    return render(request, 'prime/view-products.html',{'products':Product.objects.all(),'main_categories':MainCategory.objects.all()})

@user_passes_test(is_prime)
def main_category(request):
    return render(request, 'prime/main-category.html',{'trending_products':Product.objects.filter(trending=True)[:4],'main_categories':MainCategory.objects.all()})

@user_passes_test(is_prime)
def sub_category(request,name):
    return render(request, 'prime/sub-category.html',{
...
def products(request,category):
    if request.user.is_authenticated and request.user.groups.filter(name='prime').exists():
        products = []
        ...
        if len(products)==0:
            for i in Product.objects.filter(main_category=str(category).lower()).all():
                products.append(i)
        return render(request, 'prime/view-products.html', {'products': products,'main_categories':MainCategory.objects.all()})
...
def product_detail(request,id):
    if request.method == 'POST':
        if request.user.is_authenticated:
            if request.user.groups.filter(name='prime').exists():
                ...
                return redirect('prime-product-detail',id)
    ...
    return render(request, 'prime/product-details.html',{'product': product})

@user_passes_test(is_prime)
def profile(request):
    if request.user.is_authenticated:
        return render(request, 'prime/profile.html', {'user': request.user,'role':get_role_name(request)})
    else:
